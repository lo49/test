...
uint8_t eth_type_is_ip_and_protocol(uint8_t *buf,uint16_t len,uint8_t protocol){
    //  
    if (len<41){
        return(0);
    }
    ...
    if(buf[IP_PROTO_P] != protocol){
        return(0);
    }
    return(protocol);
}

// On vérifie si il s'agit bien d'une demande IP.
// On vérifie si il s'agit d'un broadcast ou alors notre IP , si c'est aucun des 2 on retourne 0.
uint8_t eth_type_is_ip_and_my_ip(uint8_t *buf,uint16_t len){
    ...
    while(i<4){
        if(buf[IP_DST_P+i]!=ipaddr[i] && buf[IP_DST_P+i] != 0xFF){
            return(0);
        }
        i++;
    }
...
uint8_t eth_type_is_ip_and_my_mac(uint8_t * buf,uint16_t len)
{
      ...
      uint8_t freebox[6] = {0xf4,0xca,0xe5,0x52,0xf0,0x3a};
        ...
        while(i<4){
        if(buf[ETH_SRC_MAC+i]!=freebox[i] || get_udp_port_dst(buf) != 68){
            return(0); // c'est pas la freebox !
        }
        i++;
      }
    //eth+ip+udp header is 42
...
void make_ip_udp_new(uint8_t* buf,uint16_t len,uint8_t *dst_ip){
    uint8_t i=0;
...
void make_ip_icmp_new(uint8_t* buf,uint16_t len,uint8_t *dst_ip){
    uint8_t i=0;
...
// make a new tcp header from a received tcp packet
// rel_ack_num is how much we must step the seq number received from the
// other side. We do not send more than 255 bytes of text (=data) in the tcp packet.
// If mss=1 then mss is included in the options list
//
...
// If cp_seq=0 then an initial sequence number is used (should be use in synack)
// otherwise it is copied from the packet we received
void make_new_tcphead(uint8_t *buf,uint16_t rel_ack_num,uint8_t mss,uint8_t cp_seq,uint16_t dst_port,uint16_t src_port)
{
    uint8_t i=0;
    uint8_t tseq;

    buf[TCP_DST_PORT_H_P] = dst_port>>8 & 0xFF;
    buf[TCP_DST_PORT_L_P] = dst_port & 0xFF;
    buf[TCP_SRC_PORT_H_P] = src_port>>8 & 0xFF; //set source port
    buf[TCP_SRC_PORT_L_P] = src_port &0xFF;
...
void make_echo_request(uint8_t* buf,uint8_t * dst_ip,uint8_t * dest_mac){
    make_eth_ip_new(buf,dest_mac);
    make_ip_icmp_new(buf,74,dst_ip);
    buf[ICMP_TYPE_P] = ICMP_TYPE_ECHOREPLY_V;
    // we changed only the icmp.type field from request(=8) to reply(=0).
    // we can therefore easily correct the checksum:
    if (buf[ICMP_CHECKSUM_P] > (0xff - 0x08)) {
    ...
    fill_tcp_data(buf,0, "const char *s");
...
void make_tcp_syn(uint8_t *buf,uint8_t * mac_routeur,uint8_t * ip_distant,uint16_t dst_port,uint16_t src_port){
    uint16_t ck;
    ...
    buf[TCP_FLAG_P] = TCP_FLAGS_SYN_V;
    make_new_tcphead(buf, 1, 1, 0,dst_port,src_port);
...
uint16_t get_udp_port_src(uint8_t *buf){
...
uint16_t fill_tcp_data_p(uint8_t *buf,uint16_t pos, const char *data)
{
    char c;
...
// make a  tcp header
void tcp_client_send_packet(uint8_t *buf,uint16_t dest_port, uint16_t src_port, uint8_t flags, uint32_t ack_num, 
...
*/
    buf[TCP_SEQ_P + 3] = (uint8_t) 0xff & seq_num;
    buf[TCP_SEQ_P + 2] = (uint8_t) 0xff & seq_num>>8;
    buf[TCP_SEQ_P + 1] = (uint8_t) 0xff & (seq_num>>8)>>8;
    buf[TCP_SEQ_P + 0] = (uint8_t) 0xff & ((seq_num>>8)>>8)>>8;


    buf[TCP_SEQACK_P + 3] = (uint8_t) 0xff & ack_num;
    buf[TCP_SEQACK_P + 2]=  (uint8_t) 0xff & (ack_num>>8);
    buf[TCP_SEQACK_P + 1] = (uint8_t) 0xff & ((ack_num>>8)>>8);
    buf[TCP_SEQACK_P + 0] = (uint8_t) 0xff & (((ack_num>>8)>>8)>>8);
...
uint16_t udp_get_dlength ( uint8_t *buf )
{
    int dlength;

    dlength = ( buf[ UDP_LEN_H_P ] <<8 ) | ( buf[ UDP_LEN_L_P ] );
