...
static uint16_t ip_identifier=0x51A4;
...
// Make : 
//		ETH : 
extern void make_eth(uint8_t *buf);
...
extern void make_ip_icmp_new(uint8_t* buf,uint16_t len,uint8_t *dst_ip);
//		UDP :
extern void make_udp_reply_from_request(uint8_t *buf,char *data,uint8_t datalen,uint16_t port);
extern void make_ip_udp_new(uint8_t* buf,uint16_t len,uint8_t *dst_ip);
//		TCP :
extern void make_ip_tcp_new(uint8_t *buf, uint16_t len,uint8_t *dst_ip);
extern void make_tcphead(uint8_t *buf,uint16_t rel_ack_num,uint8_t mss,uint8_t cp_seq);
extern void make_new_tcphead(uint8_t *buf,uint16_t rel_ack_num,uint8_t mss,uint8_t cp_seq,uint16_t dst_port,uint16_t src_port);
